...
int main(int argc, char *argv[]) {
    ...
    if (edit_mode == EXTRACT) {
        ...
        printf("extracted save slot to %s\n", save_slot_path.c_str());
    }
    ...
    if (edit_mode == REPLACE) {
        ...
        save->saveEncrypted(save_file_buf.data(), true);

        save_file_stream.seekp(0, std::ios::beg);
        save_file_stream.write((const char*)save->data, save_size);
        printf("replaced save slot in %s\n", save_file_path.c_str());
    }
